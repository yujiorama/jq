...
env.globals['navigation'] = collections.OrderedDict({
    'tutorial': [
        {
            'label': 'Tutorial',
            'path': 'tutorial',
...
def generate_file(env, fname='content/1.tutorial/default.yml'):
    ...
    ctx.update(unique_ctr=itertools.count(
        1), permalink=permalink, slug=slug, navitem=path)
    os.makedirs(output_dir, exist_ok=True)
    env.get_template(template_name).stream(
        ctx).dump(output_path, encoding='utf-8')
